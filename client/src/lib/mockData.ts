...
export async function getCompanies(): Promise<Company[]> {
  const response = await fetch("https://companies-compass-1.onrender.com/api/companies");
  if (!response.ok) {
...
export async function updateCompany(id: string, data: CompanyFormData): Promise<Company> {
  const response = await fetch(`https://companies-compass-1.onrender.com/api/companies/${id}`, {
    method: "PUT",
    headers: {
      "Content-Type": "application/json",
    },
